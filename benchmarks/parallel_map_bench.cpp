...
namespace
{
  ...
  BenchResult run_parallel_map_bench(
      ...
      std::size_t chunkSize)
  {
    ...
    std::vector<std::uint64_t> values(itemCount);
...
int main()
{
  ...
  std::cout << "Vix threadpool parallel_map benchmark\n";
  std::cout << "threads: " << threadCount << '\n';
  std::cout << "items: " << itemCount << '\n';
  std::cout << '\n';

  const BenchResult smallChunks =
      run_parallel_map_bench(threadCount, itemCount, 64);

  const BenchResult mediumChunks =
      run_parallel_map_bench(threadCount, itemCount, 1024);

  const BenchResult largeChunks =
      run_parallel_map_bench(threadCount, itemCount, 8192);
